 ...
 * Improved error-handling of `\Core\Util\HttpFoundation\Session\Storage\Handler\PdoSessionHandler`
 ...
 * [BC BREAK] Moved all session related classes and interfaces into own namespace, as
   `Core\Util\HttpFoundation\Session` and renamed classes accordingly.
   Session handlers are located in the subnamespace `Core\Util\HttpFoundation\Session\Handler`.
 * SessionHandlers must implement `\SessionHandlerInterface` or extend from the
   `Core\Util\HttpFoundation\Storage\Handler\NativeSessionHandler` base class.
 ...
 * Added `Core\Util\HttpFoundation\Attribute\AttributeBag` to replicate
   attributes storage behaviour from 2.0.x (default).
 * Added `Core\Util\HttpFoundation\Attribute\NamespacedAttributeBag` for
