					...
					<v-row>
						<v-col cols="12">
							<v-text-field id="name" v-model="name" :label=" $t('products.name') " @keyup="uppercase"
								:placeholder=" $t('products.product_name_placeholder') " required></v-text-field>
							<v-text-field type="number" v-model="price" :label=" $t('products.price') " value="1"
								min="1" step="0.01" required></v-text-field>
							...
							<v-select v-model="measurementUnit" :items="$store.getters['measurementUnits/getAll']"
								:label=" $t('products.measurement_unit') " item-text="name"
								item-value="id_unit_measurement" required></v-select>
							<v-select v-model="category" :items="$store.getters['categories/getAll']"
								:label=" $t('products.category') " item-text="name" item-value="id_category" required>
							</v-select>
							<v-switch color="primary" v-model="itbis"
							...
							<v-divider></v-divider>
							<v-switch color="primary" v-model="favorite"
