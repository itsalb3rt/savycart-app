	...
	<div>
		<v-row>
			<v-col cols="12">
				...
				<v-tabs fixed-tabs>
					<v-tab @click="showFavorites = false">
						...
						{{ $t('products.all') }}
						{{ !showFavorites ? filterProducts.length : null }}
					</v-tab>
			...
			<v-col v-if="loading" cols="12" class="text-center">
				<v-skeleton-loader v-for="i in 10" :key="i" class="mx-auto my-2" max-width="95%" max-height="150"
					type="card" />
			</v-col>
			<v-col cols="12" v-else-if="filterProducts.length > 0">
				<v-virtual-scroll :bench="benched" :items="filterProducts" item-height="170"
					:height="screenHeightForProductContainer">
					<template v-slot:default="{ item }">
						<list-product :product="item" :currency="currency" @view-details="
						  (product) =>
						    $router.push({
						      name: 'view_product',
						...
						" @on-delete="
						  (product) => showDialogToDeleteProduct(product.id_product)
						" @toggle-favorite="(product) => toggleFavorite(product)" :loading-favorite="loadingFavorite" />
					</template>
				</v-virtual-scroll>
			</v-col>
			<v-col v-else-if="filterProducts.length == 0" cols="12">
				<p class="headline mt-5 grey--text text-center font-weight-bold">
