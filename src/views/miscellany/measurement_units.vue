				...
				<v-card flat>
					<v-card-text>
						...
						<v-layout row wrap>
							...
							<v-flex xs12>
								<v-data-table :headers="headers" :items="$store.getters['measurementUnits/getAll']">
		...
		<v-layout row justify-center>
			<v-dialog v-model="showDialogDeleteMeasurementUnit" persistent full-width>
				<v-card>
					<v-card-title
						class="headline"
					>{{ $t('call_action_buttons.delete') }} {{ $t('measurement_unit.measurement') }}</v-card-title>
					...
					<v-card-actions>
						...
						<v-btn
							color="primary"
							flat
							@click="showDialogDeleteMeasurementUnit = false"
			...
			if (this.online) {
				this.$store
					.dispatch('measurementUnits/delete', {
						id: this.$store.getters['measurementUnits/getAll'][this.indexMeasurementUnitForDelete].id_unit_measurement
					})
					.then(response => {
						this.$store.commit(
					...
					.catch(function(error) {
						...
						console.log(
							'TCL: deleteMeasurementUnit -> error',
							error
						);
					});
			} else {
